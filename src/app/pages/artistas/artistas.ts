...
export class ArtistsComponent implements OnInit {
 ...
 guardarArtista(): void {
    ...
    const esEdicion = this.editando && this.artistaSeleccionado;
    const id = this.artistaSeleccionado?.idArtista;

    // Cerrar formulario
    this.cerrarFormulario();

    if (esEdicion) {
        this.artistaService.actualizarArtista(id!, datos).subscribe({
            next: () => {
                this.cargarArtistas();
            },
            error: (err) => console.error('Error:', err)
        ...
        this.artistaService.crearArtista(datos).subscribe({
            next: () => {
                this.cargarArtistas();
            },
            error: (err) => console.error('Error:', err)
