...
export class EquipmentComponent implements OnInit {
  ...
  guardarEquipo(): void {
    ...
    const esEdicion = this.editando && this.equipoSeleccionado;
    const id = this.equipoSeleccionado?.idEquipo;
    ...
    if (esEdicion) {
      // EDITAR
      this.equipoService.actualizarEquipo(id!, this.formulario).subscribe({
        next: () => {
          this.cargarEquipos();
        },
        error: (err) => {
